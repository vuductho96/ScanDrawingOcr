...
function Write-InspectionSampleResults($sheet,$row,$rowData,$sampleStart,$sampleEnd){
    ...
    for($sampleIndex = 1; $sampleIndex -le $sampleCount; $sampleIndex++){
        ...
        if(-not [string]::IsNullOrWhiteSpace($manualActual)){
            # Try to write manual value as number; fall back to text if it cannot be parsed
            $manualParsed = 0.0
            if([double]::TryParse($manualActual,[System.Globalization.NumberStyles]::Any,[System.Globalization.CultureInfo]::InvariantCulture,[ref]$manualParsed)){
                Set-ExcelCellNumericValue $sheet $row $targetColumn $manualParsed
            } else {
                Set-ExcelCellTextValue $sheet $row $targetColumn $manualActual
            }
            continue
        }
        $val = Get-ExportMeasurementValueUnique $rowData $absoluteSampleIndex $usedValues
        if($val -ne $null){
            Set-ExcelCellNumericValue $sheet $row $targetColumn $val
        }
    }
}
